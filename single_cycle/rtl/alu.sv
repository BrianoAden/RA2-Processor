...
(input [data_width-1:0] operand_1, input [data_width-1:0] operand_2, input [4:0] shift_amount, input ctrl_sig_pkg::ctrl_sigs_t control_sigs, 
input [2:0] func_3_bits, input logic func_7_bit, output logic [data_width-1:0] result);
...
// Need to decide whether operands and output should be signed or unsigned, had it as unsigned before

// Maybe also work on parameterization of the ALU to support different bit widths
    ...
    if (control_sigs.load_sig || control_sigs.store_sig || control_sigs.branch_sig || control_sigs.jal_sig || control_sigs.jalr_sig || control_sigs.auipc_sig) begin // all are signed
        ...
        case(alu_state_pkg::alu_state_t'(func_3_bits))
            ...
            SLTI_SLT: begin // SLTI and SLT
                result = ($signed(operand_1) < $signed(operand_2)) ? {data_width{1'b1}} : {data_width{1'b0}};
            end
            SLTIU_SLTU: begin // SLTIU and SLTU, may need to change this to unsigned comparison
                result = (operand_1 < operand_2) ? {data_width{1'b1}} : {data_width{1'b0}};
            end
